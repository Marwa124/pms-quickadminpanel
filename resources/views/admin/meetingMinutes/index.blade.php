...
@can('meeting_minute_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.meeting-minutes.create') }}">
                        ...
                        <tr data-entry-id="{{ $meetingMinute->id }}">
                            ...
                            <td>
                                @can('meeting_minute_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.meeting-minutes.show', $meetingMinute->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('meeting_minute_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.meeting-minutes.edit', $meetingMinute->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('meeting_minute_delete')
                                    <form action="{{ route('admin.meeting-minutes.destroy', $meetingMinute->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('meeting_minute_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.meeting-minutes.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-MeetingMinute:not(.ajaxTable)').DataTable({ buttons: dtButtons })
