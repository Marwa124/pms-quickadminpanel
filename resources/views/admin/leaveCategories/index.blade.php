                        ...
                        <tr data-entry-id="{{ $leaveCategory->id }}">
                            ...
                            <td>
                                @can('leave_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.leave-categories.show', $leaveCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('leave_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.leave-categories.edit', $leaveCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('leave_category_delete')
                                    <form action="{{ route('admin.leave-categories.destroy', $leaveCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
