...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.return-stocks.update", [$returnStock->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="premissions">{{ trans('cruds.returnStock.fields.premissions') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('premissions') ? 'is-invalid' : '' }}" name="premissions[]" id="premissions" multiple>
                    @foreach($premissions as $id => $premissions)
                        <option value="{{ $id }}" {{ (in_array($id, old('premissions', [])) || $returnStock->premissions->contains($id)) ? 'selected' : '' }}>{{ $premissions }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.returnStock.fields.premissions_helper') }}</span>
            </div>
            <div class="form-group">
