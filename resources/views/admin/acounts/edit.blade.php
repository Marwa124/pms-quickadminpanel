...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.acount.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.acounts.update", [$acount->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.acount.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $acount->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $acount->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('balance') ? 'is-invalid' : '' }}" type="text" name="balance" id="balance" value="{{ old('balance', $acount->balance) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('premissions') ? 'is-invalid' : '' }}" name="premissions[]" id="premissions" multiple>
                    @foreach($premissions as $id => $premissions)
                        <option value="{{ $id }}" {{ (in_array($id, old('premissions', [])) || $acount->premissions->contains($id)) ? 'selected' : '' }}>{{ $premissions }}</option>
                    @endforeach
                </select>
